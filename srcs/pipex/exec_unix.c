...
void ft_exec(t_prev prev, t_pipex *pip, int i, char **envp)
{
    ...
    if (pid == 0)
    {
        close(pip->fd[0]); // Close the read end of the pipe in the child process
...
void ft_loop(t_pipex *pipex, t_prev *prev, char **envp)
{
    while (++prev->i < pipex->cmd_count)
    {
        ...
        // else if (prev->i == pipex->cmd_count - 1 && !ft_strncmp(pipex->cmd_args[prev->i][0], "exit", 4))
        else if (prev->i == 0 && !ft_strncmp(pipex->cmd_args[prev->i][0], "exit", 4))
		{
